...
// DISPLAY QUIZ
const quizDisplay = (questionCount: number) => {
...
function quizCreator() {
  ...
  const optionButtons = quizContainer.querySelectorAll(
    ".option-div"
  ) as NodeListOf<HTMLButtonElement>;
  optionButtons.forEach((btn) => {
    btn.addEventListener("click", () => checker(btn));
  });
}

function checker(userOption: HTMLButtonElement): void {
  let userSolution = userOption.innerText;
  let question = document.getElementsByClassName("container-mid")[
    questionCount
  ] as HTMLDivElement;
  let options = question.querySelectorAll(
    ".option-div"
  ) as NodeListOf<HTMLButtonElement>;
